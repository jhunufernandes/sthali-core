...
Variables:
    ...
    DOCS_PATH (Path): The path to the docs directory.
    TEMPLATES_PATH (Path): The path to the templates directory.
    TEMPLATES (Jinja2Templates): Jinja2 templates for rendering documentation.

Functions:
    to_snake_case(string: str) -> str: Converts a given string to snake case.
    read_pyproject(path: Path | None) -> dict[str, Any]: Reads the pyproject.toml file and returns its
        content as a dictionary.
    get_imports_from_module(module: ModuleType, level: int) -> list[Doc]: Recursively extract documentation
...
def _get_docstring(obj: Any) -> str:
    metadata: list[Any] = getattr(obj, "__metadata__", [])
    field_info = next(filter(lambda x: isinstance(x, FieldInfo), metadata), None)
    doc = getattr(obj, "__doc__", None)
    return getattr(field_info, "description", None) or doc or ""
